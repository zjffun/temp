...
const availableOptions = prettier.getSupportInfo().options.map(augmentOption);
...
function PrettierOption({ options, setOptions }) {
  const enabledOptions = orderOptions(availableOptions, ENABLED_OPTIONS);

  function handleOptionValueChange(option, value) {
    setOptions((o) => {
      const options = { ...o };
...
function getCodeSamples(parser) {
  switch (parser) {
    case "babel":
    case "espree":
    case "meriyah":
...
export const defaultOptions = getDefaults(availableOptions, ENABLED_OPTIONS);

export default PrettierOption;
